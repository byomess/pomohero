...
export const NextFocusInput: React.FC = () => {
    const {
        nextFocusPlans, addNextFocusPlan, removeNextFocusPlan, updateNextFocusPlan,
    ...
    const handleAddClick = () => { if (newPlan.trim() && !isReadOnly) { addNextFocusPlan(newPlan.trim()); setNewPlan(''); } };
    ...
    const handleRemoveClick = (index: number) => { if (!isReadOnly) removeNextFocusPlan(index); };
    const handleEditStart = (index: number) => { if (isReadOnly) return; setEditingIndex(index); setEditText(nextFocusPlans[index]); playSound('click'); };
        ...
        <div className="space-y-2 flex flex-col h-full"> {/* Reduced space-y */}
            ...
            <div className="flex items-center space-x-2 flex-shrink-0">
                ...
                <button
                    ...
                    disabled={isReadOnly || !newPlan.trim()} aria-label="Adicionar plano" title="Adicionar"
                ...
                {nextFocusPlans.length > 0 ? (
                    <ul className="space-y-1.5"> {/* No absolute positioning needed */}
                        {nextFocusPlans.map((plan, index) => (
                            <li key={`nextplan-${index}-${plan}`} className={listItemBaseStyle}>
                                {editingIndex === index ? (
                                    <>
                                        <input
                                    ...
                                    <>
                                        ...
                                        > {plan} </span>
                                        <button type="button" onClick={() => handleRemoveClick(index)} className={itemDeleteButtonStyle} disabled={isReadOnly} aria-label={`Remover plano: ${plan}`} title="Remover"> <FiTrash2 className="h-4 w-4" /> </button>
                                    </>
                                )}
                            </li>
                        ))}
                     ...
                     !isReadOnly && ( // Show empty state only if editable
                         // Positioned within the flow, not absolutely
